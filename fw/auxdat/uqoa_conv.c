...
// ffmpeg -i $NAME.wav -ar 48000 -ac 1 -f s16be -acodec pcm_s16be - | ./uqoa_conv > $NAME.bin
// Consider `-af 'pan=mono|c0=FL'` instead of `-ac 1`, if straight mixdown cancels sounds

// Output: blocks of 256 uint64's
// [0] - history (all big-endian)
// [1] - weights
// [2 .. 255] - slices, each uint64 represents 20 samples
  ...
  while (!eof) {
    ...
    if (sample_count == 20) {
      if (slice_count == 0) {
        ...
        qoa_start_frame(&lms);
        put_u64(
          (uint64_t)(uint16_t)lms.history[3] << 48 |
          (uint64_t)(uint16_t)lms.history[2] << 32 |
          (uint64_t)(uint16_t)lms.history[1] << 16 |
          (uint64_t)(uint16_t)lms.history[0] <<  0
        );
        put_u64(
          (uint64_t)(uint16_t)lms.weights[3] << 48 |
          (uint64_t)(uint16_t)lms.weights[2] << 32 |
          (uint64_t)(uint16_t)lms.weights[1] << 16 |
          (uint64_t)(uint16_t)lms.weights[0] <<  0
        );
      }
      uint64_t slice = qoa_encode_slice(&lms, samples, &sf);
      put_u64(slice);
      sample_count = 0;
      if (++slice_count == 254) slice_count = 0;
    }
  }
}

// Decode
// ./uqoa_conv d < $NAME.bin | ffmpeg -ar 48000 -ac 1 -f s16le -acodec pcm_s16le -i - $NAME.re.wav
...
void decode()
{
  ...
  while (!eof) {
    ...
    for (int i = 0; i < 4; i++)
      lms.history[i] = (int16_t)((history >> (i * 16)) & 0xFFFF);
    uint64_t weights = get_u64();
    for (int i = 0; i < 4; i++)
      lms.weights[i] = (int16_t)((weights >> (i * 16)) & 0xFFFF);
    for (int n_slice = 0; n_slice < 254 && !feof(stdin); n_slice++) {
